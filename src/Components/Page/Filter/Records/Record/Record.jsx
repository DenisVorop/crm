...
const Record = ({ name, age, sex, status, lastRecordsData, timeObj, id, items, card_num, getCardNum }) => {
    ...
    if (red !== null) {
        const objPatCard = { name, age, sex, status, id, card_num }
        getCardNum(objPatCard)
        return <Navigate to={`card=${card_num}`} /> // TODO сделать по запросу вывод инфы '?card=${card_num}&'
    }
        ...
        <>
            <div className="records__row">
                <div className="records__person">
                    ...
                    <div className="records__column name-column"><p onClick={onRedirectToPatientCard}>{name}</p></div>
